...
!function (root, factory) {
    ...
    var message, options, IS_MAC = /Mac/.test(navigator.userAgent), KEY_CMD = IS_MAC ? 91 : 17,
    ...
    return MicroEvent.mixin(Selectize), void 0 !== MicroPlugin ? MicroPlugin.mixin(Selectize) : (message = "Dependency MicroPlugin is missing", options = (options = {explanation: 'Make sure you either: (1) are using the "standalone" version of Selectize, or (2) require MicroPlugin before you load Selectize.'}) || {}, console.error("Selectize: " + message), options.explanation && (console.group && console.group(), console.error(options.explanation), console.group && console.groupEnd())), $.extend(Selectize.prototype, {
        setup: function () {
            ...
            var d, tmp, self = this, query = $.trim(self.$control_input.val()), results = self.search(query),
            ...
            self.lock();
            var fn, called,
                setup = "function" == typeof self.settings.create ? this.settings.create : function (input) {
                    var data = {};
