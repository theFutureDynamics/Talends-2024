...
/* @var AttachmentsField $lstField */
/* @var Fieldable $lstModel */
?>
<lst-attachments-field
        ...
        title-text="<?php esc_html_e('Select or Upload Files', 'listivo-core'); ?>"
        button-text="<?php esc_html_e('Use this file', 'listivo-core'); ?>"
        :dependency-terms="props.dependencyTerms"
        :initial-attachments="<?php echo htmlspecialchars(json_encode($lstField->getAttachments($lstModel)->map(static function ($attachment) {
...
>
    ...
    >
        ...
        <div class="tdfm-field-group__field">
            <div v-if="fieldProps.attachments.length > 0">
                <lst-draggable
                        class="tdfm-form__media-container"
                        :list="fieldProps.attachments"
                >
                    <div
                            v-for="attachment in fieldProps.attachments"
                            :key="attachment.id"
                            class="tdfm-media-card"
                    >
                        <div class="tdfm-media-card__label">
                            {{ attachment.filename }}
                        </div>

                        <button
                                class="tdfm-media-card__close"
                                @click.prevent="fieldProps.delete(attachment.id)"
                        >
            ...
            >
                <?php esc_html_e('Add files', 'listivo-core'); ?>
