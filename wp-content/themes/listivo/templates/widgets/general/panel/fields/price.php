...
use Tangibledesign\Framework\Models\PanelFields\PricePanelField;

/* @var PricePanelField $lstPanelField */
/* @var PriceField $lstPriceField */
global $lstPanelField;
$lstPriceField = $lstPanelField->getField();

foreach (tdf_currencies() as $lstCurrency) :/* @var \Tangibledesign\Framework\Models\Currency $lstCurrency */ ?>
    <lst-price-model-field
            class="listivo-field-group"
            :model="modelForm.model"
            :field="<?php echo htmlspecialchars(json_encode($lstPanelField->getField())); ?>"
            price-key="<?php echo esc_attr($lstPriceField->getKey() . '_' . $lstCurrency->getKey()); ?>"
            :decimal-places="<?php echo esc_attr($lstCurrency->getDecimalPlaces()); ?>"
            decimal-separator="<?php echo esc_attr($lstCurrency->getDecimalSeparator()); ?>"
            :dependency-terms="modelForm.dependencyTerms"
            :selected-term-ids="modelForm.taxonomyFieldsValueIds"
            :is-indian-currency="<?php echo esc_attr($lstCurrency->isIndian() ? 'true' : 'false'); ?>"
    >
        <div
        ...
        >
            <label
                    ...
                    for="<?php echo esc_attr($lstPriceField->getKey() . '_' . $lstCurrency->getKey()); ?>"
            >
            ...
            <div class="listivo-field-group__field">
                ...
                >
                    <input
                            id="<?php echo esc_attr($lstPriceField->getKey() . '_' . $lstCurrency->getKey()); ?>"
                            name="<?php echo esc_attr($lstPriceField->getKey() . '_' . $lstCurrency->getKey()); ?>"
                            type="text"
                            :value="field.price"
                            @input="field.setPrice($event.target.value)"
                            placeholder="<?php echo esc_attr($lstPanelField->getPlaceholder()); ?>"
